...
Shadow mode now also auto-generates threshold suggestions (`profile_shadow_suggestions.json/html`) for `shadow.window_runs` and `shadow.min_match_ratio`.
Optional stable auto-apply can be enabled with `shadow.suggestion_auto_apply_enabled = true`, which only promotes a recommendation after it remains unchanged for `shadow.suggestion_stability_runs` consecutive accepted runs and persists effective settings in `profile_shadow_suggestion_state.json` (with run-by-run history in `profile_shadow_suggestions_history.csv`).
The product dashboard SYSTEM page surfaces both the current recommendation and the active applied threshold state in the shadow panel.
