...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run ablation study comparing v1-like control vs v2 setup across walk-forward folds."
    )
    parser.add_argument("--data", required=True, help="Input OHLCV CSV path")
    parser.add_argument("--models-dir", required=True, help="Output directory for ablation model snapshots")
    parser.add_argument("--report", required=True, help="CSV path for per-fold ablation metrics")
    parser.add_argument("--summary", required=True, help="CSV path for variant-level ablation summary")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Downside-volatility penalty used by v2 variant",
    )
    parser.add_argument(
        "--v2-drawdown-penalty",
        type=float,
        default=0.05,
        help="Drawdown-spike penalty used by v2 variant",
    )
    parser.add_argument(
        "--downside-window",
        type=int,
        default=20,
        help="Rolling window used to compute downside volatility",
    )
    parser.add_argument(
        "--position-levels",
    ...
    metrics = run_ablation_study(
        ...
        seed=args.seed,
        v2_downside_risk_penalty=args.v2_downside_penalty,
        v2_drawdown_penalty=args.v2_drawdown_penalty,
