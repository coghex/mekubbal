...
from mekubbal.file_lock import locked_path
...
def run_profile_rollback(
    *,
    ...
    with locked_path(selection_path):
        ...
        effective_run_time = (
            str(run_timestamp_utc) if run_timestamp_utc is not None else run_times[-1]
        )
        if effective_run_time not in run_times:
            raise ValueError(
                f"run_timestamp_utc not found in health history: {effective_run_time}"
            )
        ...
        if bool(rollback_on_ensemble_events):
            if ensemble_alerts_history_path is None:
                raise ValueError(
                    "ensemble_alerts_history_path is required when "
            ...
            if not ensemble_history.empty:
                if (
                    "symbol" not in ensemble_history.columns
                    or "run_timestamp_utc" not in ensemble_history.columns
                ):
                    raise ValueError(
                        "Ensemble alerts history must include symbol and "
                        "run_timestamp_utc columns."
                    )
        ...
        target_profile = str(
            ...
            else promotion_rule.get(
                "base_profile", promotion_rule.get("fallback_profile", "base")
            )
        )
        ...
        last_rollback_at = (selection.get("rollback") or {}).get("applied_at_utc")
        if last_rollback_at is not None:
            runs_since_rollback = sum(
                1 for value in runs_up_to_latest if value > str(last_rollback_at)
            )
        else:
            runs_since_rollback = cooldown_runs  # no prior rollback — never in cooldown
        for symbol_raw, active in sorted(active_profiles.items()):
            ...
            latest_ensemble_alert = bool(
                ensemble_alert_lookup.get((symbol, effective_run_time), False)
            )
            ...
            in_cooldown = bool(
                cooldown_runs > 0 and runs_since_rollback < cooldown_runs
            )
            should_rollback = bool(
                (drift_triggered or ensemble_triggered) and not in_cooldown
            )
            rollback_action = "none"
            if in_cooldown and (drift_triggered or ensemble_triggered):
                rollback_action = "suppressed_cooldown"
            ...
            symbol_rows.append(
                {
                    ...
                    "min_consecutive_ensemble_event_runs": int(
                        min_consecutive_ensemble_event_runs
                    ),
        ...
        rollback_path.write_text(
            json.dumps(rollback_state, indent=2, sort_keys=True), encoding="utf-8"
        )
