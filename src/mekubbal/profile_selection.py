...
def run_profile_promotion(
    ...
    *,
    ...
    with locked_path(state_file):
        ...
        existing_active = (
            existing_state.get("active_profiles", {}) if isinstance(existing_state, dict) else {}
        )
        ...
        for symbol in symbols:
            ...
            if symbol_rows.empty:
                continue
            symbol_rows["symbol_rank"] = pd.to_numeric(
                symbol_rows["symbol_rank"], errors="coerce"
            )
            symbol_rows["avg_equity_gap"] = pd.to_numeric(
                symbol_rows["avg_equity_gap"], errors="coerce"
            )
            ...
            by_profile = {
                str(row["profile"]): row
                for _, row in symbol_rows.sort_values("symbol_rank").iterrows()
            }
            candidate_row = by_profile.get(candidate_profile)
            base_row = by_profile.get(base_profile)
            previous_active = (
                str(existing_active.get(symbol)) if symbol in existing_active else None
            )
            reasons: list[str] = []
            ...
            if promote:
                ...
                if prefer_previous_active and previous_active in by_profile:
                    ...
                    active_profile = str(
                        symbol_rows.sort_values("symbol_rank").iloc[0]["profile"]
                    )
