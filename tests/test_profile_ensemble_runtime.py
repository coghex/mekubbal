...
def test_prepare_ensemble_v3_dedupes_same_run_timestamp(tmp_path):
    pairwise_csv = tmp_path / "pairwise.csv"
    pd.DataFrame(
        [
            {
    ...
    history = pd.read_csv(ensemble_history_path)
    assert len(history) == 1
    assert history.iloc[0]["run_timestamp_utc"] == "2026-01-04T00:00:00+00:00"
